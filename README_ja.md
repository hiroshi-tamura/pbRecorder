...
- **ウィンドウ録画に他のウィンドウが被り込まなくなりました** — ウィンドウモードが `PrintWindow` から **Windows.Graphics.Capture (WGC)** に移行。WGC は OS レベルで対象ウィンドウの合成前サーフェスを取得するため、上に重なった別ウィンドウは一切録画されず、`PrintWindow` では黒くなっていた GPU 描画（ブラウザ・ゲーム・動画プレイヤー）も正しく録れます。WGC 非対応の Windows では従来の `PrintWindow` にフォールバック。静止ウィンドウでも一定フレームレートで出力します。
- **アプリ単位の音声が実際に録れるようになりました** — プロセスループバックがどの環境でも activation に失敗していました。完了ハンドラがフリースレッドマーシャリング未対応で `ActivateAudioInterfaceAsync` が `E_ILLEGAL_METHOD_CALL` を返し、`AUDCLNT_STREAMFLAGS_AUTOCONVERTPCM` 無しでフォーマットが拒否され、UWP/ストアアプリでは誤ったプロセス（`ApplicationFrameHost.exe`）を対象にしていました。この3点を修正し、**「▶ 対象ウィンドウのアプリ音声のみ」** が対象アプリ（と子プロセス）の音声を意図通り録音します。
- **ウィンドウ録画開始時のクラッシュを修正** — 非再帰の `std::mutex` を同一スレッドで二重ロックしていた（エラー処理が呼び出し側の保持中と同じ mutex をロック）ためプロセスが落ちていました。全キャプチャ/音声ソースで修正。
- **ビルド**: WGC/C++WinRT のため MSVC ツールチェインでビルドするようになりました。配布物に Visual C++ ランタイムを同梱しているため、別途インストール不要（従来通り解凍して実行）。Windows 10 1903 以降（アプリ単位音声は build 20348 以降）が必要です。

## 0.5.8 の新機能

- **長時間録画での音ズレを解消** — 映像のタイムスタンプを、音声と同じ高分解能クロック（QPC）の実キャプチャ時刻に固定するようにしました。従来はフレームごとに「1/FPS 秒」を機械的に加算しており、映像クロックが実時間より先行したまま戻らず、録画が長くなるほど音と絵がどんどんズレていました。現在は音声・映像が単一のクロックを共有するため、録画の最後まで同期が保たれます。全コンテナ（MP4 / WMV / MKV）に適用。

## 0.5.7 の新機能

- **アプリ単位の音声録音** — ウィンドウモードで、キャプチャ対象ウィンドウのアプリ（と子プロセス）の音声だけを録音できます（Windows のプロセスループバック API を使用）。出力デバイス一覧で **「▶ 対象ウィンドウのアプリ音声のみ」** を選択します（ウィンドウモード時のみ有効）。
- **ウィンドウ録画のズレ修正** — ウィンドウモードで生じていた微妙なズレを修正。不可視のリサイズ枠分ずれず、ウィンドウの可視（DWM）領域にぴったり切り出します。ウィンドウモードは `PrintWindow` 使用のため、対象に他ウィンドウが重なっても録画には入りません。
- **コンパクトな固定サイズ UI** — メイン画面を高密度な2カラム構成（ソース＋映像／音声＋出力）に刷新。ドラッグでのリサイズ不可で中身に密着（余白ゼロ）、PCM 等で行が増える時やプレビュー表示時だけ高さが伸びます。プレビューは既定で非表示、**プレビュー** チェックで表示します。

## 主な機能

### キャプチャモード
- **画面全体** — ディスプレイ全体を録画（マルチモニター対応）
- **ウィンドウ指定** — 特定のウィンドウだけを録画（`PrintWindow` 使用のため重なった他ウィンドウは映らず、可視領域にぴったり切り出し）
- **範囲指定** — 画面の任意の矩形領域を録画
  - **オートアジャスト機能** — ウィンドウの境界線を自動検出し、選択範囲をスナップ
  - 選択後もエッジをドラッグして微調整可能
- **UI領域追跡** — マウス下の UI Automation 要素を選択し、移動しても録画範囲を追跡
  - `Ctrl+Space`、`Enter`、クリックでハイライト中の UI 領域を確定、`Esc` でキャンセル
  - **親ウィンドウから切り出し** はデフォルト ON。別ウィンドウの重なりが録画に入りにくくなります
  - 対象アプリが `PrintWindow` に対応しない場合は、画面からの切り出しに戻せます
...
- **アプリ単位ループバック**: 対象ウィンドウのアプリ音声だけを録音（ウィンドウモード時。Windows のプロセスループバック API を使用、Windows 10 build 20348+ / Windows 11）
...
1. [Releases](https://github.com/pbtechlab/pbRecorder/releases)からZIPをダウンロード
2. 任意のフォルダに展開
3. `pbRecorder.exe`（GUI）または `pbRecorder-cli.exe`（CLI）を実行

## GUI の使い方

1. キャプチャソースを選択（画面全体/ウィンドウ/範囲指定/UI領域追跡）
2. 必要に応じて音声デバイスを選択
3. コンテナ形式・コーデックを設定
4. 出力先フォルダとファイル名を設定
5. 録画ボタン（または `Ctrl+R`）で録画開始/停止

UI領域追跡では、**UI領域追跡** を選び、**UI領域選択** を押して対象のペインやコントロールにマウスを合わせ、ハイライトされた領域を確定します。別ウィンドウの重なりを避けたい場合は **親ウィンドウから切り出し** を ON にします。

### ショートカットキー
- `Ctrl+R` — メインウィンドウから録画開始/停止
- `Ctrl+Shift+R` — グローバル録画開始/停止ホットキー
- 範囲選択時: `Enter` で確定、`Esc` でキャンセル
- UI領域選択時: `Ctrl+Space`、`Enter`、クリックで確定、`Esc` でキャンセル、マウスホイール/矢印キーで近傍の親UI領域へ切り替え

### UI領域追跡の注意点

UI領域追跡は GUI 専用機能です。Windows UI Automation で選択したペインやコントロールの矩形を取得し、その領域を録画します。デフォルト ON の **親ウィンドウから切り出し** は、親ウィンドウを直接キャプチャしてから UI 領域を切り出すため、別ウィンドウの重なりを避けやすくなります。一部のアプリでは `PrintWindow` の描画が黒くなる、古い表示になる、または正しく更新されない場合があります。その場合はチェックを OFF にして画面表示どおりの切り出しに戻してください。

制限事項: UI領域追跡は対象アプリの UI Automation ツリーとウィンドウ描画の挙動に依存します。独自描画、管理者権限、非表示、最小化状態のアプリでは安定した UI 領域を取得できない場合があります。

## CLI の使い方

`pbRecorder-cli.exe` はコマンドラインから全録画機能を利用できます。
UI領域追跡は対話的な UI 要素選択が必要なため GUI 専用です。
